...
5. **自检**（32 项断言）：
   ```bash
   node verify.mjs ./output/文章.html            # 新文章先在 verify.mjs 的 PROFILES 里加 profile
   ```
   输出结尾应为 `全部通过（共 32 项）`（详见「verify.mjs 用法」）。
6. **浏览器预览 QA**（可选但推荐）：
   ```bash
...
- `image-map.json` 里同时保留了两期的映射：02《谁有权给一个概念命名？》（4 张，文件名即原图名）与 03《你的蓝色和我的蓝色一样吗？》（6 张：封面 `JustThinking_03.png` 与 `03-01/03-02/03-04/03-05/03-06` 图），可作为新文章的模板。
...
- 行内 `[^n]` → 上标 `<sup>[n]</sup>`（深金 `#b8860b`）；**数字脚注沿用原编号**；
- 行内 `[^name]`（Obsidian 默认的具名脚注）→ 同样渲染为上标，**按「首次引用顺序」自动编号 1,2,3…**（自动编号会跳过文档中数字脚注已占用的编号，因此纯数字脚注的旧文章产物不受影响）；
- 文末 `[^n]: 内容` / `[^name]: 内容` 定义 → 收集为「参考资料」区块，按编号排序，**插回首个脚注定义出现的位置**（保持「正文 → 参考资料 → ■ → AI 声明」的顺序）；
- 脚注定义中的 URL 自动着色为深金（与正文链接一致）；定义里的 `**粗体**` / `*斜体*`（如书目题名）照常渲染；定义文本**不**做直角引号转换（保留原文）；
- 站内脚注区标题（`#### 引用资料…`、`## 参考文献`）会被「参考资料」块替代，不再单独渲染。
...
**脚注（5 项）**：上标引用条数；脚注定义条数；引用编号均在 1..定义数；定义编号 1..定义数 齐全；每个被引用编号都有定义。

**图注与间距（3 项）**：图注条数 = profile 值；有图注时校验「图底 12px + 图注上距 0」，无图注文章则校验「全部图片统一 `margin: 20px auto`」且无图注样式残留。
...
- `<input.html>`：转换产物（相对路径或绝对路径都可以，脚本内部统一转成 `file:` URI）
...
- **verify.mjs 与文章绑定**：32 项断言中的数量（图片张数、上标/定义条数、图注条数、脚注 URL 条数、关键内容顺序）按文章写在各 profile 里，换文章需在 `PROFILES` 里新增一份并切换 `DEFAULT_PROFILE`（见「verify.mjs 用法」）；
